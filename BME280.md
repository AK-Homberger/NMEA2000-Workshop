...
Zur Nutzung des BME280-Moduls müssen wir nun noch die Adafruit BME-2800-Bibliothek installieren. Das geht recht einfach mit dem Arduino-IDE Bibliotheksverwalter.
Hierzu im Menü "Sketch", "Bibliothek einbinden" und dann "Bibliotheken verwalten..." auswählen. Das startet den Biblitheksverwalter. In das Suchfeld "BME280" eintragen, dann "Adafruit BME280 Library" auswählen und "Installieren" drücken.


# Beispielprogramm installieren
Nun installieren wird das erste Beispielprogramm auf dem rechten ESP32. Dazu wird der ESP32 mit USB verbunden und in der Arduino-IDE der neue serielle USB-Port ausgewählt. Dann in das Verzeichnis "NMEA2000-BME280-1" des heruntergeladenen und entpackten Reepositories wechseln und auf ["NMEA2000-BME280-1.ino"]((https://github.com/AK-Homberger/NMEA2000-Workshop/blob/main/NMEA2000-BME280-1/NMEA2000-BME280-1.ino) ) doppelklicken.
...
# Definition von globalen Variablen/Objekten

Kommen wir nun zur Definition von globalen Variablen und Objekten. Globale Variablen heißen "global", weil sie in allen Funktionen verwendet werden können. Im Gegensatz zu "globalen" sind "lokale" Varibalen, die in Funktionen definiert werden, nur in der jeweiligen Funktion bekannt und nuztbar.
...
Als erstes wird ein Objekt "bme" vom Typ "Adafruit_BME280". Diese Objekt enhält die notwendigen Meethoden/Funktionen zur Verwendung des BME280-Moduls.
...
Als nächstes wird die serielle Schnittstelle (USB) mit der Baud-Rate 115200 getartet. Das ist notwendig, um den Seriellen Monitor der Arduino-IDE nutzen zu können.
...
```
Mit den obigen Zeilen wird eine eindeutige "id" aus der WLAN-Netzwerkadresse (MAC = Media Access Code) des ESP32-Moduls berechnet.
...
Die konkreten Inhalte der Produkt-Information sind für die korrekte Funktion nicht relevant. Sie dienen nur der Information und können auf anderen Gereäten (z.B. NMEA2000-Plotter) angezeigt werden. Im NMEA-Reader werden dies Informationen übrigens im Reiter "Properties" angezeigt. Allerdings nur, wenn das ESP32 Modul nach Anschluss des Readers neu gestartet wurde (RST-Taste) und der PGN 60928 "ISO Address Claim" vom Modul gesendet wurde.
...
```
Als nächstes werden die Geräte-Informationen festgelegt. In folgener Reihenfolge:

- Die berechnete eindeutige ID.
- Geräte-Funktion: 130 entspricht einem Temperatur-Sensor.
- Geräte-Klasse: 75 entprcht einem "Sensor Communication Interface".
- Freier Hersteller-Code. Wir nutzen hier 2046.

Die zu verwendenden Codes sind auf der NMEA.Org-Webseite zu finden (Link siehe oben).
...
Im nächsten Schritt setzen wir die Node-Adresse unseres Moduls. Als Standard-Adresse legen wir die Nummer 34 fest. Falls es zuvor einen Adress-Koflikt gegeben hatte, zwei Geräte wollten die gleiche Adresse 34 nutzen, handeln beide Geräte eindeutige Adressen aus. Es gehört zum NMEA2000-Standard, dass alle Geräte sich die letzte ausgehandelte Adresse merken und beim nächsten Start auch verwenden, um einen erneuten Adresskonflikt zu vermeiden.

Der ESP32 hat einen nichtflüchtigen Speicher (Flash-Memeory). Aus diesem Speicher lesen wir einen zuvor gegebenenfalls gespeicherten Wert für "LastNodeAddress" und weisen diesen der globalen Variablen "NodeAddress" zu. Wir werden später bei der Funktion CheckSourceAddressChange() noch einmal darauf zurückkommen.
...
# Notwendige Hilfsfunktionen

Als nächtes sehen wir uns die definirten Hilfsfunktionen etwas genauer an.
...
```
Für ungeübte C-Programmierer sind die Funktionen wohl recht schwer zu verstehen. Wir gehen hier auch nicht im Detail darauf ein. Wir werden aber in der nächsten Funktion darauf eingehen, wie sie zu nutzen sind.
...
```
Mit der obigen Funktion SendN2kTemperature(void) bauen wir die zu sendenden NMEA-Daten zusammen und senden sie auf den NMEA2000-Bus.
...
Die verschiedenen PGNs und eventuell nötige Datentypen sind in den beiden Dateien ["N2kMessages.h"](https://github.com/ttlappalainen/NMEA2000/blob/master/src/N2kMessages.h) und ["N2kTypes.h"](https://github.com/ttlappalainen/NMEA2000/blob/master/src/N2kTypes.h) fetgelegt.

Für das hier verwendete PGN130312 werden folgende Argumente erwartet:

1. "tN2kMsg &N2kMsg": Das ist die zuvor defienirte Variable zum Speichern der NMEA2000-Nachricht.
2. "unsigned char SID": Die Sequence ID ermöglicht die Kennzeichnung zusammengehöriger Nachrichten. Das benötigen wir hir nicht und setzen ein "0" ein.
...
5. "double ActualTemperature": Hier wird die aktuelle Temperatur in Kelvin eingetragen. Da unsere Temperatur vom BME280 in Grad Celsius vorliegtnutzen wir das Makro "CToKelvin(Temperature)" zur Umwandlung.
...
Kommen wir nun zur vorletzten Funktion in diesem Beisoiel:
...
Das machen wir mit der Funktion CheckSourceAddressChange(). Hier prüfen wir, ob sich die eigene Node-Adresse aufgrund eine Adresskonflikts geändert hat (SourceAddress != NodeAddress). Falls sich die Adresse geändert hatte, speichern wir die neue Adress im Flash-Speicherdes ESP32. Beim nächsten Start des Moduls wird die geänderte Adresse dann auch gleich wieder verwendet um einen ereuten Adresskonflikt zu vermeiden.

Die letzte Funktion is die Funktion loop(), die automatisch und wiederkehrend immer wieder aufgrufen wird.
...
2. Mit NMEA2000.ParseMessages() geben wir der NMEA2000-Biblithek regelmäcßig die Gelegnheit auf empfangene Nachrichten zu reagieren.
3. Mit CheckSourceAddressChange() prüfen wir regelmäßig, ob sich die eigene Adresse geändert hat.
4. Hier lesen wir eventuell empfangene Zeichen aus der seriellen Schnittstelle um Problemen vorzubeugen.

Hier noch ein Hinweis: In anderen Arduino-Programmen sieht man häufig die verwendung von delay() um einen Zeitraum abzuwarten. Bei NMEA2000-Programmen ist es keine gute Idee in loop() oder daraus aufgerufenen Funktionen delay() zu verwenden. Das würde den regelmäßigen Aufruch von NMEA2000.ParseMessages() verzögern und kann zu Problemen mit der zügigen Verarbeitung der Nachrichten führen.

Das war es mit dem 1. Beispielprogramm. Im nächsten Schritt werden wir das BME280-Beipielprogramm erweitern, um auch die Luftfeuchtigkeit mit einem eigenen PGN zu senden.

BME280 [Teil 2](https://github.com/AK-Homberger/NMEA2000-Workshop/blob/main/BME280-2.md)

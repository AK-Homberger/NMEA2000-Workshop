...
Die folgen Zeilen zeigen die notwendigen Inormationen für die Nutzung der DallasTemperature-Bibliothek:
...
# 3. Definition der neuen Task

Als erstes fügen wir die globale Variable "Task1" hinzu:
...
Gegenüber dem BME280-Programm sind einige Änderung durchgeführt worden.

Wir starten wir num den Temperatur-Sensor
...
```
Die Funktion GetTemperatur wird einmalig bei der Task-Erzeugung in setup() aufgrufen und darf auf keine Fall verlassen werden.
...
1-Wire erlaubt die Nutzung mehrerer Temperatursensoren an einem Bus. Wir nutzen aber hier nur einen Sensor. Daher "sensors.getTempCByIndex(0)". "0" entspricht erstem Sensor.
